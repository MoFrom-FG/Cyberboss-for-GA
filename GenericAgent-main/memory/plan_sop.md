...
按 subagent.md 启动探索subagent，**加 `--verbose`** 开启监察模式。探索subagent统一用 `--llm_no 1`（轻量模型，见 model_routing_sop.md）。input要点：
...
**启动subagent前必须读 `model_routing_sop.md`** 确定 `--llm_no`：探索subagent固定 `--llm_no 1`，验证subagent固定 `--llm_no 0`，执行subagent按任务强度判定。
...
做法：起 subagent 完成具体操作，要求返回精简摘要，主 agent 基于摘要继续决策。保持主 agent 上下文干净是第一优先级。委托时读 model_routing_sop.md 确定 --llm_no。
...
按 subagent.md 标准流程启动验证subagent（统一用 `--llm_no 0` 强模型，见 model_routing_sop.md），input要点：
