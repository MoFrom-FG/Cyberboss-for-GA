...
function normalizeSystemMessage(message) {
  ...
  const mode = normalizeSystemMessageMode(message.mode);
...
function normalizeSystemMessageMode(value) {
  const normalized = normalizeText(value).toLowerCase();
  if (normalized === "maintenance" || normalized === "companion" || normalized === "subagent_result") {
    return normalized;
  }
  return "";
}
