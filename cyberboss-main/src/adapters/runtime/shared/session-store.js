...
class SessionStore {
  ...
  getThreadIdForWorkspace(bindingKey, workspaceRoot, runtimeId = this.runtimeId) {
    ...
    return getWorkspaceMapValue(scoped, normalizedWorkspaceRoot);
  }
  ...
  getCompanionThreadId(bindingKey, runtimeId = this.runtimeId) {
    const binding = this.getBinding(bindingKey) || {};
    const normalizedRuntimeId = normalizeValue(runtimeId) || "default";
    const map = getCompanionThreadMap(binding);
    return normalizeThreadValue(map[normalizedRuntimeId]);
  }

  setCompanionThreadId(bindingKey, threadId, extra = {}, runtimeId = this.runtimeId) {
    const current = this.getBinding(bindingKey) || {};
    const normalizedRuntimeId = normalizeValue(runtimeId) || "default";
    const companionThreadIdByRuntime = {
      ...getCompanionThreadMap(current),
      [normalizedRuntimeId]: normalizeThreadValue(threadId),
    };
    return this.updateBinding(bindingKey, {
      ...current,
      ...extra,
      companionThreadIdByRuntime,
    });
  }

  clearCompanionThreadId(bindingKey, runtimeId = this.runtimeId) {
    const current = this.getBinding(bindingKey) || {};
    const normalizedRuntimeId = normalizeValue(runtimeId) || "default";
    const companionThreadIdByRuntime = {
      ...getCompanionThreadMap(current),
  ...
  getRuntimeParamsForWorkspace(bindingKey, workspaceRoot) {
    ...
    const entry = getWorkspaceMapEntry(codexParamsByWorkspaceRoot, normalizedWorkspaceRoot);
    return {
  ...
  getPendingThreadIdForWorkspace(bindingKey, workspaceRoot, runtimeId = this.runtimeId) {
    ...
    const binding = this.getBinding(bindingKey) || {};
    return getWorkspaceMapValue(getPendingThreadMapForRuntime(binding, runtimeId), normalizedWorkspaceRoot);
  }
  ...
  listWorkspaceRoots(bindingKey, runtimeId = this.runtimeId) {
    const current = this.getBinding(bindingKey) || {};
    return [...new Set(Object.keys(getThreadMapForRuntime(current, runtimeId))
  ...
  findBindingForThreadId(threadId, runtimeId = this.runtimeId) {
    ...
    for (const [bindingKey, binding] of Object.entries(this.state.bindings || {})) {
      const companionThreadId = getCompanionThreadMap(binding)[normalizedRuntimeId || "default"];
      if (normalizeValue(companionThreadId) === normalizedThreadId) {
        return {
          bindingKey,
          workspaceRoot: normalizeWorkspaceRoot(binding?.activeWorkspaceRoot),
        };
      }
  ...
  getApprovalCommandAllowlistForWorkspace(workspaceRoot) {
    ...
    const raw = getWorkspaceMapEntry(this.state.approvalCommandAllowlistByWorkspaceRoot, normalizedWorkspaceRoot);
...
function getWorkspaceMapValue(map, workspaceRoot) {
  const value = getWorkspaceMapEntry(map, workspaceRoot);
  return normalizeValue(value);
}

function getWorkspaceMapEntry(map, workspaceRoot) {
  const normalizedWorkspaceRoot = normalizeWorkspaceRoot(workspaceRoot);
  if (!map || typeof map !== "object" || !normalizedWorkspaceRoot) {
    return undefined;
  }
  if (Object.prototype.hasOwnProperty.call(map, normalizedWorkspaceRoot)) {
    return map[normalizedWorkspaceRoot];
  }
...
function getCompanionThreadMap(binding) {
  return binding?.companionThreadIdByRuntime && typeof binding.companionThreadIdByRuntime === "object"
