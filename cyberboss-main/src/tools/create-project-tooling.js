...
const { TodayTaskService } = require("../services/today-task-service");
...
function createProjectTooling(config, options = {}) {
  const sessionStore = options.sessionStore || new SessionStore({
    ...
    runtimeId: config.runtime || "genericagent",
  });
  ...
  const services = {
    ...
    todayTask: new TodayTaskService({ config }),
    wechatMaintenance: new WechatMaintenanceService({ filePath: config.wechatMaintenanceStateFile }),
